...
void receivedCallback( uint32_t from, String &msg ) {
  ...
  */
  ...
  Serial.println( String("values received ") + values[0] + "," + values[1] + "," + values[2]) ;
  ...
  // motorA(speedA );
  // motorB(speedB );
  // moveMotors(100, speedA);
  for (int i = 0; i<NUM_LEDS; i++) {
    leds[i].setHSV (values[2], 255, 50); //leds
  }
...
//
void moveMotors(int dist, int spd) {
  if (spd > 0) {
...
void bnoSend() {
  ...
  if (currentMillis - previousMillis >= BNO055_SAMPLERATE_DELAY_MS) {
    ...
    */
    ...
    //      msg = "";
    //    }
    msg = int(euler.y()) + String(",") + int(euler.z()) + String(",") + int(euler.x()) + String(",");
    Serial.println(msg);
    mesh.sendBroadcast( msg );
  }
}
